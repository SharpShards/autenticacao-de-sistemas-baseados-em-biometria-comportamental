...
function ajeitarTS() {
    ...
    let milli = String(ts.getMilliseconds()).padStart(3, '0');

    return year + "-" + month + "-" + day + " " + hour + ":" + minute + ":" + second + ":" + milli;
}

// Dados de digitação
let tecla, evento, campo, ts, epoch;
...
// Dados do mouse
let coord, event_m, ts_m, epoch_m;
let stt = true;
...
function ordemCampos(e) {
    if (!ord.includes(e.target.id)) {
        ord.push(e.target.id);
    }
}
...
salvar.addEventListener('click', async function salvarDados() {
    ...
    let ordemFormatada = ord.join("-");

    // Adicionar dados de comportamento
    amostra['comp'] = { 1: { ord: ordemFormatada, num_type, up_type, time_txt, time_form } };
    console.log(amostra)
    console.log(amostra['comp'])
    ...
    // URL da API
    let url = 'https://benrtcumaj.execute-api.sa-east-1.amazonaws.com';
